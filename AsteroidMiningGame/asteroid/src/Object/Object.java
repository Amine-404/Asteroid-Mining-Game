...
public abstract class Object {
	...
	public void set_position(int a_, int b_) {
		y = a_ * tile_size + tile_size / 2;
		x = b_ * tile_size + tile_size / 2;
		x_des = x;
		y_des = y;
		a = a_;
		b = b_;
	...
	public boolean can_move_next_position() // check if the object can move to the next position
	{
		...
		{
			if (map.getType(curr_row, curr_col - 1) == Field.blocked || curr_col == 0)
				return false;
			else
				x_des = x - tile_size;
		}
		...
		{
			if (map.getType(curr_row, curr_col + 1) == Field.blocked || curr_col == map.getNum_columns())
				return false;
			else
				x_des = x + tile_size;
		}
		if (u) // if moving up
		{
			if (map.getType(curr_row - 1, curr_col) == Field.blocked || curr_row == 0)
				return false;
			else
				y_des = y - tile_size;
		}
		if (d) // if moving down
		{
			if (map.getType(curr_row + 1, curr_col) == Field.blocked || curr_row == map.getNum_rows() - 1)
				return false;
			else
				y_des = y + tile_size;
		}
	...
	public void next_Position() {
		if (r && x < x_des)
			x += speed; // if move right and current position is less than the next position we move to
						// the next position with a certain speed
		else
			r = false;
		if (r && x > x_des)
			x = x_des;
